// app/espace-client/profil/page.tsx
...
export default async function ProfilePage() {
  ...
  const { profile, error: profileError } =
    await getClientProfile(session.id)
  const {
    collaborators,
    error: collabError,
  } = await getClientCollaborators(session.id)
      ...
      {profileError ? (
        <Card>
          <CardContent className="pt-6">
            <p className="text-muted-foreground">
              Impossible de charger les informations de profil. Veuillez réessayer plus
              tard.
            </p>
          </CardContent>
        </Card>
        ...
        <>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {profile.forme_juridique || "Non renseignée"}
                    </p>
                  </div>
                </div>
                <div className="flex items-center">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Liste des collaborateurs invités</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {collabError ? (
                <p className="text-muted-foreground">
                ...
                <div className="grid gap-6 md:grid-cols-3">
                  {collaborators.map((c) => (
                    <div key={c.id} className="space-y-2 border rounded p-4">
                      <div>
                        <p className="text-sm font-medium">Email</p>
                        <p className="text-sm text-muted-foreground">
                          {c.email}
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {c.date_invitation
                            ? new Date(c.date_invitation).toLocaleDateString(
                                "fr-FR"
                              )
                            : "—"}
                        </p>
